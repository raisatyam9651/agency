...
def get_relative_links(file_path):
    ...
    for link in links:
        ...
        # Clean query parameters or hash fragments
...
def verify_links():
    ...
    # We want to search for index.html files under base_dir (excluding _build/ and images/)
    html_files = []
    ...
    for file_path in html_files:
        ...
        # print(f"Checking {os.path.relpath(file_path, base_dir)}: found {len(rel_links)} links.")
        for link in rel_links:
            total_links_checked += 1
            # Compute absolute path of target link
            target_path = os.path.abspath(os.path.join(rel_dir, link))
            
            # Special case: check if folder is referenced (like ../haryana/index.html or ../haryana)
            # If path points to a directory, check if index.html exists in that directory
